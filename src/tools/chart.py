...
def generate_chart(
    ...
    try:
        ...
        # Return chart as MCP resource reference
        if image_file_path and image_data:
            # Return resource reference for Claude Desktop with screen: URI scheme
            return [
                types.EmbeddedResource(
                    type="resource",
                    resource=types.BlobResourceContents(
                        uri=f"screen://localhost/{chart_id}",
                        mimeType="image/png",
                        blob=image_data  # base64-encoded binary
                    )
                )
            ]
        else:
